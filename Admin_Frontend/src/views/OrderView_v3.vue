        ...
        <div class="table-responsive">
          <table class="table">
            ...
            <tbody>
              <tr v-for="order in filteredOrders" :key="order.orderId">
                ...
                <td>
                  <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deleteMenuModal"
                    v-on:click="onSelectOrder(order)" :disabled="order.status?.statusId === 7"><svg
                      xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="#dc3545" class="bi bi-trash3-fill"
                      viewBox="0 0 16 16">
                      <path
              ...
              <!-- test 訂單詳情區 -->
              <tr class="collapse" :id="'collapseOrder' + order.orderId" v-for="order in filteredOrders" :key="'detail-' + order.orderId">
                <td colspan="8">
                  <div style="border-radius: 10px; background-color: #fff3cd;">
                    <div class="container-fluid">
                      <div class="d-flex justify-content-between p-3">
                        <h5>訂購人 : <span>{{ order.userId }}</span></h5>
                      ...
                      <table class="table mb-3 table-borderless table-responsive table-warning text-start">
                        ...
                        <tbody>
                          <tr v-for="item in getOrderItems(order.orderID)" :key="item.orderID">
                            <th scope="row">{{ formatDate(order.reservation?.reservationDate) }}</th>
                            <td>x{{ order.reservation?.timePeriod?.period }}</td>
